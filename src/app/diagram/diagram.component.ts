...
export class DiagramComponent implements OnInit {
  ...
  public start() {
    ...
    this.diagram.contextMenu =
      ...
      //Guardar Posicion
      $("ContextMenuButton",
              $(go.TextBlock, "Agregar Mesa"),
              { click: (e, obj) => this.onAddMesa(e, obj) }),
    ...
    this.events.subscribe("onMesas",(data) => this.onMesas(data));

    this.events.publish("getMesas",'');
  ...
  //Agregar Nueva Mesa
  public onAddMesa(e, obj) {
    let loc = e.diagram.toolManager.contextMenuTool.mouseDownPoint;

    let dataSTR = "?userid="+ " " + "&xpos=" + parseInt(loc.x) + "&ypos=" + parseInt(loc.y);
  ...
  public onMesas(dataSTR) {
    ...
    for (let i = 0; i < data.Mesas.length; i++) {
      ...
      let obj = {
        idbox:data.Mesas[i].filters["IDBox"],
        uid:data.Mesas[i].uid,
        label_state:data.Mesas[i].label_state,
        loc: new go.Point(data.Mesas[i].loc.x , data.Mesas[i].loc.y),
        color:"#FFFFFF",
        conncolor:"#FFFFFF",
        angle:data.Mesas[i].angle,
        visible:true,
        imgsource:""
      }

      obj.imgsource = "assets/mesa.png";

      let arrTMP = data.Mesas[i].filters.CambioEstado.split(",");
      ...
      if (this.services.viewmode == "VistaEstado") {
        ...
        obj.conncolor = data.Mesas[i].color;
        if (parseInt(data.Mesas[i].filters.Online) == 0) obj.color = data.Mesas[i].filters["OnlineColor"];
        else obj.color = data.Mesas[i].filters["OnlineColor"];

      } else {
        obj.color = data.Mesas[i].color;
        if (parseInt(data.Mesas[i].filters.Online) == 0) obj.conncolor = data.Mesas[i].filters["OnlineColor"];
        else obj.conncolor = data.Mesas[i].filters["OnlineColor"];
      }

      let node;
      let valid:boolean = true;
      for (let j = 0; j < this.diagram.model.nodeDataArray.length; j++) {
        if ((this.diagram.model.nodeDataArray[j] as any).uid == data.Mesas[i].uid) {
